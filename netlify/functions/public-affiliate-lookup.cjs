...
// "FundHub × Credit Comeback Club"). Deliberately returns only the two
// display fields a landing page needs — never anything else on the
// affiliates row (email, commission_rate, payout history, etc).
exports.handler = async (event) => {
  ...
  try {
    ...
    return {
      ...
      body: JSON.stringify({
        affiliate: affiliate ? { name: affiliate.name, company: affiliate.company || null } : null
      })
    };
  } catch (err) {
