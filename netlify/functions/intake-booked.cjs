...
exports.handler = async (event) => {
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f-]{27,36}$/i.test(String(leadId || ''))) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Invalid lead' }) };
  }
  ...
  if (!lead?.email) return { statusCode: 404, body: JSON.stringify({ error: 'Lead not found' }) };

  const marker = 'consultation_booked_confirmation';
  const sent = Array.isArray(lead.lead_drips_sent) ? lead.lead_drips_sent : [];
  if (sent.includes(marker)) return { statusCode: 200, body: JSON.stringify({ success: true, duplicate: true }) };
  ...
  if (enrollment) {
    try {
      const provisionRes = await fetch(base + '/.netlify/functions/provision-user', {
        ...
        body: JSON.stringify({ email: lead.email, fullName: lead.name, kind: 'client' }),
      });
  ...
  if (!emailRes.ok) {
    ...
    return { statusCode: 502, body: JSON.stringify({ error: 'Could not send booking confirmation' }) };
  }
  ...
  return { statusCode: 200, body: JSON.stringify({ success: true, portalInvited }) };
};
