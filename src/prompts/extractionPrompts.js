...
export function combinedExtractionPrompt(chunk = null, {
  ...
  const contextNote = Number.isInteger(contextPage) && contextPage > 0
    ? ` LOCAL PAGE ${contextPage} is a context-only, source-derived bureau-column legend from original source page 1. It contains only the fixed left-to-right bureau-column layout needed to interpret DATA LOCAL PAGES ${dataPages.join(', ')}; the original page's substantive content is intentionally absent. Use the legend only to bind bureau identity to the same fixed columns on those data pages. Do not extract report dates, client/profile values, accounts, inquiries, cleanup items, or account fields from the context page. Do not mark reportSectionStart from the context legend; reportSectionStart must be false in this checkpoint. Only bureauEvidencePage may cite the context page. Every other evidence page must be one of the listed data pages.`
    : '';
