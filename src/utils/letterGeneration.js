...
/**
 * Explicit gate before Lob/mail. Throws Error with a stable code so UI can toast.
 * @param {object} letter
 */
export function assertSignatureReadyForMail(letter) {
  const state = resolveLetterSignatureState(letter);
  if (state === 'embedded' || state === 'ok' || state === 'present') {
    return true;
  }
  if (state === 'missing_signature' || state === 'signature_required') {
    const err = new Error(
      'Cannot mail: this letter has no drawn client signature embedded. Complete enrollment signature or re-generate the letter with the signature on file.'
    );
    err.code = 'MISSING_CLIENT_SIGNATURE';
    throw err;
  }
  if (state === 'generating') {
    const err = new Error('Cannot mail: letter PDF is still generating. Wait for the signature-embedded PDF to finish.');
    err.code = 'LETTER_STILL_GENERATING';
    throw err;
  }
  const err = new Error(`Cannot mail: letter signature state is "${state}". Fix the drawn signature before sending.`);
  err.code = 'SIGNATURE_STATE_BLOCKED';
  throw err;
}

/**
 * Human-readable explanation for UI banners.
 */
export function describeSignatureBlocker(letter) {
  const state = resolveLetterSignatureState(letter);
  if (state === 'embedded' || state === 'ok' || state === 'present') return null;
  if (state === 'missing_signature' || state === 'signature_required') {
    return {
      code: 'MISSING_CLIENT_SIGNATURE',
      title: 'Drawn signature required',
      message: 'No usable drawn signature is embedded on this letter. Have the client complete enrollment (draw signature), then re-generate the letter before mailing.',
    };
  }
  if (state === 'generating') {
    return {
      code: 'LETTER_STILL_GENERATING',
      title: 'Letter still generating',
      message: 'Wait until the PDF finishes generating with the signature embedded.',
    };
  }
  return {
    code: 'SIGNATURE_STATE_BLOCKED',
    title: 'Signature problem',
    message: `Letter signature state is "${state}". Resolve the drawn signature before mailing.`,
  };
}
