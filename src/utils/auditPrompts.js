...
export function mergeAuditPrompt(t, eqData, expData, tuData) {
  return `MERGE_AUDIT_JSON_MODE\n\nToday is ${t}.\n\nMerge these three bureau reports into a unified forensic audit. Match accounts across bureaus. Identify cross-bureau violations. Classify each account A/B/C. Rank top 5 as Batch 1, rest as Batch 2. For the client Recovery Blueprint PDF, you MUST populate 'primaryViolation', 'strategy', 'addressStatus' and 'furnisherAddress' for EVERY account as complete sentences (never truncated). The Opening Move is the highest-balance Batch 1 account — make that account's strategy especially concrete. Also merge all hard inquiries and personalInfo data across the bureaus.\n\nData:\n${JSON.stringify({ equifax: trimBureau(eqData), experian: trimBureau(expData), transunion: trimBureau(tuData) }, null, 2)}\n\nJSON only.`;
}
