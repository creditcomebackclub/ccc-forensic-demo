...
async function claudeCall(apiKey, systemPrompt, userContent, maxTokens = 8192) {
...
export async function runAudit(pdfBase64) {
  ...
  const rawText = await claudeCall(apiKey, MASTER_SYSTEM_PROMPT, pdfContent(
    pdfBase64,
    `AUDIT_JSON_MODE\n\nToday is ${today()}. Perform a full forensic Metro 2 and FCRA audit of the attached three-bureau credit report. Return the complete JSON object per the schema in your instructions. Identify every violation. Classify accounts A, B, or C. Rank into Batch 1 top 5 and Batch 2 remaining. Output JSON only. No prose. No code fences.`
  ));
...
export async function runTripleBureauAudit(eqBase64, expBase64, tuBase64, onProgress) {
  ...
  const eqText = await claudeCall(apiKey, MASTER_SYSTEM_PROMPT, pdfContent(eqBase64, bureauPrompt('Equifax')));
  const eqData = extractJSON(eqText);

  onProgress && onProgress('Waiting 20 seconds (rate limit)...', 25);
  await sleep(20000);

  onProgress && onProgress('Analyzing Experian report...', 35);
  const expText = await claudeCall(apiKey, MASTER_SYSTEM_PROMPT, pdfContent(expBase64, bureauPrompt('Experian')));
  const expData = extractJSON(expText);

  onProgress && onProgress('Waiting 20 seconds (rate limit)...', 50);
  await sleep(20000);

  onProgress && onProgress('Analyzing TransUnion report...', 60);
  const tuText = await claudeCall(apiKey, MASTER_SYSTEM_PROMPT, pdfContent(tuBase64, bureauPrompt('TransUnion')));
  const tuData = extractJSON(tuText);

  onProgress && onProgress('Waiting 20 seconds (rate limit)...', 75);
  await sleep(20000);

  onProgress && onProgress('Computing cross-bureau violations...', 80);

  const mergePrompt = `MERGE_AUDIT_JSON_MODE\n\nToday is ${t}.\n\nMerge these three bureau reports into a unified forensic audit. Match accounts across bureaus by furnisher name and account number. Identify cross-bureau violations: balance discrepancies, payment history conflicts, date mismatches, status conflicts. Classify each account A/B/C. Rank top 5 by violation strength as Batch 1, rest as Batch 2. Return complete audit JSON matching your standard schema.\n\nData:\n${JSON.stringify({ equifax: trimBureau(eqData), experian: trimBureau(expData), transunion: trimBureau(tuData) }, null, 2)}\n\nJSON only. No prose.`;

  const mergeText = await claudeCall(apiKey, MASTER_SYSTEM_PROMPT, [{ type: 'text', text: mergePrompt }], 8192);
...
export async function runSingleBureauAudit(pdfBase64, bureau) {
  ...
  const rawText = await claudeCall(apiKey, MASTER_SYSTEM_PROMPT, pdfContent(
    pdfBase64,
    `AUDIT_JSON_MODE\n\nToday is ${today()}. Bureau: ${bureau} only. Perform a forensic Metro 2 and FCRA audit. No cross-bureau comparisons possible. Return complete JSON per standard schema. JSON only.`
  ));
  const json = extractJSON(rawText);
  if (json && json.client) await saveAudit(json);
...
export async function generateLetter(account, client) {
  const apiKey = getApiKey();
  const t = today();
  const rawText = await claudeCall(apiKey, MASTER_SYSTEM_PROMPT, [{
    type: 'text',
