...
export async function generateLetter(account, client) {
  ...
  if (isTypeC) {
    const baseInstructions = `LETTER_HTML_MODE\n\nToday is ${t}. Use this exact date at the top of the letter.\n\nData:\n${baseData}\n\nIf clientSignature is provided embed it in the signature block. This letter is mailed standard USPS First Class, not certified — do NOT include a "Certified Mail #" or any tracking/article number field or placeholder, and do NOT write "Certified Mail" anywhere — state only "Sent via First Class Mail" with no number. Output complete HTML only. No prose. No fences.`;
  ...
  const instructions = `LETTER_HTML_MODE\n\nToday is ${t}. Use this exact date at the top of the letter.\n\nGenerate the Phase 1 dispute letter HTML for this account.\n\nData:\n${JSON.stringify({ account, client, clientSignature: client.signatureData || null }, null, 2)}\n\nFollow the 16-step structure. If clientSignature is provided embed it in the signature block. This letter is mailed standard USPS First Class, not certified — do NOT include a "Certified Mail #" or any tracking/article number field or placeholder, and do NOT write "Certified Mail" anywhere — state only "Sent via First Class Mail" with no number. Output complete HTML only. No prose. No fences.`;
